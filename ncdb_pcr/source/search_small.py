...
fi_ranking = pd.read_csv('data/shap_values.csv', index_col=0)
fi_list = list(fi_ranking.index)
top5 = fi_list[:5]
...
plt.xlim([0.0, 1.0])
plt.savefig('results/roc_5.png', dpi=300, bbox_inches='tight')
plt.clf()
...
plt.xlim([0.0, 1.0])
plt.savefig('results/prc_5.png', dpi=300, bbox_inches='tight')
plt.clf()

with open('results/results_5.txt', 'w') as f:
    f.write('AUROC' + '\n')
    f.write(rf_label + '\n')
    f.write(xgb_label + '\n')
    f.write(nn_label + '\n')
...
senspec.to_csv('results/senspec_5.csv')